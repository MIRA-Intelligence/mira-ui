...
| 模式 | 说明 |
|------|------|
| **Desktop App** | Electron 打包的本地桌面应用，连接本地 radiologybot |
| **Cloud Web App** | 纯 Web 应用（Nginx 托管），用户通过浏览器访问远程 radiologybot |
...
| 项 | 值 |
|---|---|
| 源码位置 | `/Users/cwang/Code/RadiologyBot/radiologybot/` |
| 包名 | `radiologybot-ai 0.1`（pip editable install） |
| Conda 环境 | `nanobot` |
| CLI 入口 | `radiologybot` |
| 关键命令 | `radiologybot gateway` / `radiologybot agent -m "msg"` |
...
radiologybot agent -m "your message"
```

### 2.3 核心架构：异步消息总线

radiologybot gateway **没有 HTTP API**。它是纯异步消息总线架构：
...
源码：`radiologybot/channels/base.py`
...
源码：`radiologybot/session/manager.py`
...
源码：`radiologybot/config/schema.py`
...
源码：`radiologybot/channels/manager.py`

每个 channel 在 `_init_channels()` 中按以下模式注册：

```python
if self.config.channels.xxx.enabled:
    from radiologybot.channels.xxx import XxxChannel
...
| 方案 | 描述 | 评估 |
|------|------|------|
| A. 新增 WebChannel | 在 radiologybot 中新增 `channels/web.py`，遵循 BaseChannel 接口 | **最佳** — 架构最干净 |
| B. 外部 API Bridge | 独立 FastAPI 进程，通过某种 IPC 连接 bus | 复杂度高，需跨进程通信 |
| C. CLI 子进程 | Electron 通过 spawn 调用 `radiologybot agent` | 仅适用 Desktop，无法 Cloud 部署 |
...
| **后端新增** | aiohttp (WebChannel) | 嵌入 radiologybot gateway，无额外进程 |
...
│   └── docker-compose.yml           # 前端 + radiologybot 编排
...
Cloud 模式下 Nginx 反向代理到 radiologybot gateway :18790。
...
> **目标**：让 radiologybot gateway 暴露 WebSocket + REST API
...
| docker-compose | frontend (:80) + radiologybot gateway (:18790) |
...
# ===== 后端 =====
conda activate nanobot
radiologybot gateway                     # 启动 gateway + WebChannel :18790
